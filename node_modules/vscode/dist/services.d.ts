...
declare type ExtensionUntrustedWorkspaceSupport$1 = {
    supported: true;
} | {
    supported: false;
    description: string;
...
declare type ExtensionVirtualWorkspaceSupport$1 = boolean | {
    supported: true;
} | {
...
declare type NotificationMessage = string | Error;
...
declare type IAnyWorkspaceIdentifier = IWorkspaceIdentifier | ISingleFolderWorkspaceIdentifier | IEmptyWorkspaceIdentifier;
...
declare type IConfigurationUpdateOverrides = Omit<IConfigurationOverrides, 'overrideIdentifier'> & {
 ...
 */
declare class Codicon implements CSSIcon {
    readonly id: string;
    readonly definition: IconDefinition$1;
...
declare type JSONSchemaType = 'string' | 'number' | 'integer' | 'boolean' | 'null' | 'array' | 'object';
...
declare type IconIdentifier = string;
declare type IconDefaults = ThemeIcon | IconDefinition;
 ...
 * @internal
 */
declare type InlineValue = InlineValueText | InlineValueVariableLookup | InlineValueExpression;
/**
...
declare type Definition = Location | Location[] | LocationLink[];
/**
...
declare type ContextKeyValue = null | undefined | boolean | number | string | Array<null | undefined | boolean | number | string> | Record<string, null | undefined | boolean | number | string>;
interface IContext {
 ...
 */
declare type IStringDictionary<V> = Record<string, V>;

/**
 * Configuration options for auto closing quotes and brackets
 */
declare type EditorAutoClosingStrategy = 'always' | 'languageDefined' | 'beforeWhitespace' | 'never';
/**
 * Configuration options for auto wrapping quotes and brackets
 */
declare type EditorAutoSurroundStrategy = 'languageDefined' | 'quotes' | 'brackets' | 'never';
/**
 * Configuration options for typing over closing quotes or brackets
 */
declare type EditorAutoClosingEditStrategy = 'always' | 'auto' | 'never';
/**
...
declare type GoToLocationValues = 'peek' | 'gotoAndPeek' | 'goto';
/**
...
interface IEditorExperimentalOptions {
    /**
...
declare type QuickSuggestionsValue = 'on' | 'inline' | 'off';
/**
...
declare type LineNumbersType = 'on' | 'off' | 'relative' | 'interval' | ((lineNumber: number) => string);
...
declare type InUntrustedWorkspace = 'inUntrustedWorkspace';
/**
...
interface IBaseKeybinding extends Modifiers {
    isDuplicateModifierCase(): boolean;
}
declare class SimpleKeybinding implements IBaseKeybinding {
    readonly ctrlKey: boolean;
    readonly shiftKey: boolean;
    readonly altKey: boolean;
    readonly metaKey: boolean;
...
declare type KeybindingModifier = 'ctrl' | 'shift' | 'alt' | 'meta';
/**
...
declare type IEditorModel = ITextModel | IDiffEditorModel;
/**
 ...
 */
declare type IEditorViewState = ICodeEditorViewState | IDiffEditorViewState;
...
declare type IGDPRPropertyWithoutMetadata<T> = Omit<T, 'owner' | 'comment' | 'expiration'>;
declare type ClassifiedEvent<T extends IGDPRProperty> = {
    [k in keyof IGDPRPropertyWithoutMetadata<T>]: any;
};
declare type StrictPropertyChecker<TEvent, TClassifiedEvent, TError> = keyof TEvent extends keyof TClassifiedEvent ? keyof TClassifiedEvent extends keyof TEvent ? TEvent : TError : TError;
declare type StrictPropertyCheckError = {
    error: 'Type of classified event does not match event properties';
};
declare type StrictPropertyCheck<T extends IGDPRProperty, E> = StrictPropertyChecker<E, ClassifiedEvent<T>, StrictPropertyCheckError>;
declare type GDPRClassification<T> = {
    [_ in keyof T]: IPropertyData | IGDPRProperty | undefined | string;
};
...
declare type DialogType = 'none' | 'info' | 'error' | 'question' | 'warning';
interface ICheckbox {
    readonly label: string;
...
interface IInputResult extends IShowResult {
    /**
...
interface IDialogService {
     ...
     * option then promise with index `0` is returned.
     */
    show(severity: Severity$1, message: string, buttons?: string[], options?: IDialogOptions): Promise<IShowResult>;
    /**
     ...
     */
    input(severity: Severity$1, message: string, buttons: string[], inputs: IInput[], options?: IDialogOptions): Promise<IInputResult>;
    /**
...
declare type LanguageSelector = string | LanguageFilter | ReadonlyArray<string | LanguageFilter>;
...
declare type IBulkEditPreviewHandler = (edits: ResourceEdit[], options?: IBulkEditOptions) => Promise<ResourceEdit[]>;
...
declare class SemanticTokensProviderStyling {
...
declare type DocumentTokensProvider = DocumentSemanticTokensProvider | DocumentRangeSemanticTokensProvider;
...
declare type IFileOpenOptions = IFileOpenForReadOptions | IFileOpenForWriteOptions;
...
declare type OpenOptions = OpenInternalOptions & OpenExternalOptions;
declare type ResolveExternalUriOptions = {
    readonly allowTunneling?: boolean;
};
...
declare type ConfigurationSyncStore = {
    url: string;
    insidersUrl: string;
    stableUrl: string;
    canSwitch: boolean;
...
declare type ExtensionVirtualWorkspaceSupport = {
...
declare type ImportantExtensionTip = {
    name: string;
    languages?: string[];
    pattern?: string;
...
declare type QuickPickInput<T = IQuickPickItem> = T | IQuickPickSeparator;
...
declare abstract class EditorInput extends AbstractEditorInput {
     ...
     */
    resolve(): Promise<IEditorModel$1 | null>;
    /**
...
declare type ACTIVE_GROUP_TYPE = typeof ACTIVE_GROUP;
/**
 * Open an editor to the side of the active group.
 */
declare const SIDE_GROUP = -2;
declare type SIDE_GROUP_TYPE = typeof SIDE_GROUP;
...
declare type IWindowOpenable = IWorkspaceToOpen | IFolderToOpen | IFileToOpen;
...
declare type SaveSource = string;
...
declare type IUntypedEditorInput = IResourceEditorInput | ITextResourceEditorInput | IUntitledTextResourceEditorInput | IResourceDiffEditorInput | IResourceSideBySideEditorInput | IResourceMergeEditorInput;
...
declare type GroupIdentifier = number;
...
declare type ICloseEditorsFilter = {
    except?: EditorInput;
...
interface IEditorGroupsService {
     ...
     */
    readonly onDidLayout: Event<IDimension$1>;
    /**
...
declare class MenuId {
    ...
    static readonly MergeToolbar: MenuId;
...
declare class SubmenuItemAction extends SubmenuAction {
    ...
    private readonly _options?;
    constructor(item: ISubmenuItem, _menuService: IMenuService, _contextKeyService: IContextKeyService, _options?: IMenuActionOptions | undefined);
    get actions(): readonly IAction[];
}
...
declare class CompositeMenuActions extends Disposable {
    ...
    private readonly contextMenuActionsDisposable;
...
interface IViewPaneContainer {
    onDidAddViews: Event<IView$2[]>;
    onDidRemoveViews: Event<IView$2[]>;
    onDidChangeViewVisibility: Event<IView$2>;
...
declare class ViewMenuActions extends CompositeMenuActions {
    constructor(element: HTMLElement, viewId: string, menuId: MenuId, contextMenuId: MenuId, donotForwardArgs: boolean, contextKeyService: IContextKeyService, menuService: IMenuService, viewDescriptorService: IViewDescriptorService);
}
...
declare type IExtensionPointHandler<T> = (extensions: readonly IExtensionPointUser<T>[], delta: ExtensionPointUserDelta<T>) => void;
...
declare type ExtensionRunningLocation = LocalProcessRunningLocation | LocalWebWorkerRunningLocation | RemoteRunningLocation;
...
declare type PanelAlignment = 'left' | 'center' | 'right' | 'justify';
...
declare class ViewPaneContainer extends Component implements IViewPaneContainer {
    ...
    readonly onDidAddViews: Event<IView$2[]>;
    private readonly _onDidRemoveViews;
    readonly onDidRemoveViews: Event<IView$2[]>;
    private readonly _onDidChangeViewVisibility;
    readonly onDidChangeViewVisibility: Event<IView$2>;
    private readonly _onDidFocusView;
    readonly onDidFocusView: Event<IView$2>;
    private readonly _onDidBlurView;
    readonly onDidBlurView: Event<IView$2>;
